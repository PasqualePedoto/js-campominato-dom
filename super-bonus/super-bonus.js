...
// # SUPER BONUS - RICORDARE DI AGGIUNGERE QUESTO SCRIPT NELL'HTML
...
const onClickShowNearCells100 = (element) => {
    console.log(element.innerText);
    if ((parseInt(element.innerText) > 0) /*&& (parseInt(element.dataset.number) % 10 === 0) && (parseInt(element.dataset.number) < 91)
        && (((parseInt(element.dataset.number) + 9) % 10 === 0))*/ ) {
        const upNumber = document.querySelector(`.cell-${parseInt(element.innerText) - 10}`);
        if (isBomb(upNumber)) upNumber.classList.add('_clicked');
    }
}
...
const battleCamp10 = () => {
    for (let i = 1; i <= 100; i++) {
        ...
        let tracker = `cell-${i}`;
        element.classList.add('_cells-100', tracker); // * Aggiunta per realizzare il bonus
        element.innerText = i;
        ...
        element.addEventListener('click', (event) => {
            if (element.classList.contains('_clicked')) return;
                ...
                // userPoints++;
                onClickShowNearCells100(element);
